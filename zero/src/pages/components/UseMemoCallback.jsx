...
function UseMemoCallback() {
    ...
    // 해당 함수를 다시 연산하고 그 외의 경우에는 연산하지 않는다. 즉, 기존값을 유지한다.

    // useCallback은 함수자체를 가지고 있는것
    // useCallback은 연산의 문제가 아닌 선언의 문제
    const onChangeText = useCallback(() => {
        // 기존의 값에서 반대되는 것을 바꿔준다는 의미
        // true에서 반대되는 것이 false이므로 누르면 false → true → false 반복
        // memo가 true인 상태로 스코프에 등록 그리고 해당 값을 유지하고 재선언하지 않는다.
    ...
    // memo와 callback의 차이점은 memo는 연산 후에 값이 저장
